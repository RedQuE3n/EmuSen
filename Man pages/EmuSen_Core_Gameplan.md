...
**Multi-core intent:** `Cores/Snes/` is already folder-scoped assuming a future `Cores/Nes/` sibling. The debug toolchain (`Debug/IDebugTarget.cs` and everything built on it) is already genuinely core-agnostic by design — the one piece of the multi-core intent that's actually been built and proven useful, not just planned. The C# namespaces underneath `Cores/Snes/` haven't caught up to the folder structure yet (still `EmuSen.Memory`, not `EmuSen.Cores.Snes.Memory`) — a known, deliberately-deferred cleanup.
...
- **The drive-by namespace rename** (`EmuSen.Memory` → `EmuSen.Cores.Snes.Memory`, etc.) — cheap, zero-behavior-change. Do it opportunistically when touching affected files rather than as its own dedicated pass.
