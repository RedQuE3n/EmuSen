...
**Confirmed mechanism** (verified with per-pixel diagnostics — see `Renderer.Scanline.cs`'s `ColorMathBlendLogging`/`ColorMathBlendScanline` and `Renderer.Backgrounds.cs`'s `[BG1TILE]` trace, both added during this investigation):
...
**The only reset path found:** one state (`$00:DA63`) in Bank00's overworld module-load state machine, which runs once during a full module transition (house interior → overworld) and re-derives `$1D` from the destination overworld area number (`$8A`): zero unless `$8A` is one of `{$00,$70,$40,$5B,$03,$05,$07,$43,$45,$47}`. Confirmed via `--watch WRAM:0x1D:1:write` that ordinary overworld walking/scrolling (no module transition) never writes `$1D` at all across 400+ frames — so nothing else in normal gameplay can un-stick it once `RestoreTorchBackground` has set it outdoors.

**Open question — not fully resolved:** whether real hardware also gets stuck here (i.e. this is a genuine, obscure vanilla ALTTP bug, and EmuSen would be correctly replicating it) or whether EmuSen additionally fails to run the `$00:DA63` reset state on the house-exit transition (a second, EmuSen-specific bug stacked on top of the first). A save state taken "just outside the house" (immediately after leaving, before any further movement) already showed `$1D` stuck despite `$8A` not being in the allow-list above — which is what the reset state should have zeroed if it ran. Time/tooling constraints (no cycle-accurate reference emulator on hand, and the headless harness's `--script` commands only run once at the very end of a `RunFrame()` batch rather than interleaved with it, so a watch can't be armed mid-run to catch a fresh transition without also being fed via `--watch` at process start) meant this specific sub-question was never directly settled. **If revisiting:** capture a state from literally the frame before the house-exit transition, drive the exit with `--tap`, and check whether PC ever reaches `$00:DA63` during it (e.g. via `break add`'s hit-count, since headless `break` doesn't halt — see `BreakCommand`'s own comment).

**Current fix (`VenusCore.ApplyLttpOverworldSubscreenQuirk`):** rather than resolve the open question above, EmuSen now defensively re-runs the *same* check the game's `$00:DA63` state performs, every frame, while the overworld module is active (`WRAM $10 == 0x09`): if `$8A` isn't in the allow-list and `$1D != 0`, force it to `0`. This is a no-op whenever `$1D` is already correct, so it can't affect the legitimate lamp/dark-room torch effect (which runs through a different mechanism, in different modules, entirely untouched by this check) — it only ever *corrects* the stuck-outdoors case, on whatever frame it happens, regardless of which of the two theories above turns out to be true. Scoped to this one game via `Cartridge.Title.StartsWith("THE LEGEND OF ZELDA")`; `Cartridge.Title` is `[SkipInState]` (derived fresh from ROM bytes every `LoadRom()`, deliberately excluded from `StateSerializer`'s positional field walk so adding it doesn't desync save states made before it existed — see that field's own comment for why this bit specifically).

**To find an accurate fix later:** if it's ever confirmed real hardware does NOT get stuck (i.e. this is purely an EmuSen module-transition bug), the right fix is to find and correct whatever's actually wrong with the `$00:DA63` dispatch on the house→overworld transition, and `ApplyLttpOverworldSubscreenQuirk` can then be deleted — the defensive per-frame check exists only because that root cause was never pinned down, not because it's the preferred long-term shape of the fix.
