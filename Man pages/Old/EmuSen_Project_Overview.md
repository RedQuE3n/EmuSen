...
## 2. Current features, by subsystem
...
## 3. Known, already-accepted architectural simplifications (not bugs — don't re-flag without new evidence)
...
4. **Watchpoints beyond WRAM** — wire `WatchRegistry.RecordWrite` into `Ppu`'s VRAM/CGRAM/OAM writes and the general CPU-bus/SRAM path.
...
6. **Audio output** — connect the already-correct S-DSP synthesis to an actual playback device.
7. **Decimal (BCD) mode** on the 65816 (ADC/SBC currently ignore the D flag).
8. **The stuck HDMA title-screen window bug** — dedicated investigation, now that windowing is confirmed safe to leave on globally.
9. **Breakpoints / single-step / pause-resume** — needs real execution-loop support for pausing mid-frame, which doesn't exist today. A real, separate piece of work, not a quick add.
10. **The Avalonia GUI debug window** — the actual Mesen-style multi-pane debugger, built against `IDebugTarget` once enough of the above exists to make it worthwhile.
11. **A second `IDebugTarget` implementation (NES or otherwise)** — to actually prove out the core-agnostic design rather than just asserting it.
12. **True doubled-resolution interlace output** — explicitly deferred (§2) given its rarity in real games versus its engineering cost; revisit only if a specific ROM actually needs it.
13. **NES core work generally** — the original long-term goal this whole architecture (folder structure, `IDebugTarget`, the eventual namespace cleanup) has been building toward, still not started. Planned to begin only once SNES + the Avalonia frontend are stable — per earlier project discussion, not a new decision.
