...
**A real bug caught building `AudioPlayerTests.cs`, not a hypothetical one:** `Environment.SetEnvironmentVariable("SDL_AUDIODRIVER", "dummy")` does **not** reach the real libc environment on this runtime - verified directly by calling `getenv(3)` via P/Invoke immediately afterward and getting back an empty string. Every SDL-touching test failed with `IsAvailable == false` (SDL falling through to ALSA, absent in a sandboxed test environment, instead of the dummy driver) until the test's static constructor called `setenv(3)` via P/Invoke directly instead. Flagged here in case the same shape of bug (`Environment.SetEnvironmentVariable` not being visible to a native library's own `getenv()` call in the same process) shows up again anywhere else native interop and environment variables meet.
