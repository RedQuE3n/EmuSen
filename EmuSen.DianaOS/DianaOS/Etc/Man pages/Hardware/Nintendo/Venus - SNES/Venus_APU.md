...
### 1.6 Fixed bug: the SPC700 ran past its cycle budget, so port writes became visible to the CPU up to a whole instruction early

**Symptom.** Illusion of Gaia (`IoG.smc`) booted, played its intro and title screen, then hung on a permanent black screen the moment Start was pressed. The 65816 sat forever in a two-instruction spin at `$82:8BAC` (`LDA $2140` / `BNE`), waiting for the sound driver to acknowledge command `$F0` ("stop music") by writing 0 back to port 0, with `INIDISP` = `$80` (forced blank) — hence black rather than frozen-on-the-title.

**Why the hang was a symptom, not the bug.** IoG's `$F0` handler (SPC `$0636`) opens with `MOV A,$D5` / `BEQ` — if voice 0 has no active track pointer it returns *without* acknowledging. That path is unreachable on real hardware because the title music is playing when the game asks to stop it. Under EmuSen no music was playing: `channels` reported `KeyOns=0` on all eight voices for the entire run, so the driver's per-frame DSP flush wrote `KON` = 0 forever and the ack never came. The CPU-side spin loop had no timeout, so a silent driver became a hard hang.

**Root cause.** `Step()` executed a whole instruction whenever `CycleBudget > 0`, so it could overshoot its budget by that instruction's full cost (up to 12 cycles). Anything the SPC700 wrote in that instruction therefore became visible to the 65816 up to a full instruction ahead of real hardware time.

That is fatal for the audio-upload handshake IoG uses. The game uploads its sound driver with the IPL ROM's own transfer loop, which reads the data byte (`$FFDE MOV A,$F5`) *before* acknowledging (`$FFE0 MOV $F4,Y`) — no window, and that block arrives byte-perfect. It then uploads song and sample data through a second-stage loader it relocates into APU RAM at `$0F97`, whose loop acknowledges *first* (`$0F9F MOV $F4,Y`) and only then reads the byte (`$0FA1 MOV A,$F5`). On hardware the SPC700 gets there 3 cycles after the ack (`MOV $F4,Y` is 4, `MOV A,$F5` is 3) while the CPU needs ~12 to poll, bump its counter and issue the next 16-bit `STA $2140` — comfortable. With the ack arriving early, the CPU's next write landed while the SPC700 was still sitting at `$0FA1`, clobbering `$F5` before it was read.

**Signature in the data.** A lost byte does not desync the protocol — the ack is driven by the SPC700's own counter, not the payload — so the block arrives at exactly the right length with the wrong contents. Byte `N` is replaced by byte `N+1`, and byte `N+1` then appears again on the next iteration. Diffing APU RAM against the ROM source (found via `LDA [$46],Y` at `$82:91B8`, pointer at WRAM `$46-$48`) showed exactly that: the 24-byte instrument table destined for `$1300` arrived as `32 65 7f b2 cb cb e5 fc …` against a true `32 65 7f 98 b2 cb e5 fc …`, and 1810 of the 4444 song bytes at `$1400` were wrong. Garbage song data means garbage track pointers, which means no notes, which means no key-ons.

**Fix.** `VenusCore` now runs the SPC700 only while the budget covers the next instruction's base cost (`while (Spc700.CycleBudget >= Spc700.PeekStepCycles())`), so writes can no longer become visible before the cycles that produced them have elapsed. `PeekStepCycles` deliberately reads the opcode through the IPL overlay rule but *not* through `Read8`, since `Read8` clears the timer counters at `$00FD-$00FF` as a side effect and a peek must not. A taken branch's `+2` penalty (§1.5's `_branchExtraCycles`) can still overshoot by 2, which is well inside the handshake's margin.

**Confirmation.** With `--flag ApuPortTrafficLogging` over a 500-frame boot, CPU writes to the data port landed with the SPC700 at `$0FA1` (acked, byte not yet read) 16,720 times before the fix and 0 times after; every one of the 40,820 writes now lands at `$0FA3`, past the read. IoG goes from `KeyOns=0` to 192 over 1200 frames and now walks past the title screen into gameplay. No other title in the test library changed its key-on count, and all 506 `EmuSen.WiseMan` tests still pass.

**Diagnostic worth reusing.** "CPU spins forever on `$2140`" is almost never a CPU bug — check `channels` for `KeyOns` first. A driver that is alive (its timer-wait loop is running, its track pointers advance) but silent points at corrupted uploaded data, and comparing an uploaded APU block against its ROM source is the fastest way to see it. Note the port log de-duplicates on value change, so it cannot be used to reconstruct the byte stream — use the SPC700's PC distribution at each write instead, which is what localized this.
