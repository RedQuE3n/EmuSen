...
**What it does not cover.** The two filters available are the simple single-pass ones Serenity already had; nothing here is an accurate CRT or LCD. The in-game bar's options menu does not offer the filter yet. Tests: `ScreenFilterSettingTests` (every tab has the row, a choice is saved for that console alone, a game starts with its console's filter and a change reaches it, an unknown name draws no filter); three mutants each caught.
