...
- **Linux — X11 (default), not Wayland.** Both `Program.cs` files used to call `.UseWayland()` explicitly on Linux (Avalonia doesn't pick its Wayland backend up via `UsePlatformDetect()` on its own). That opt-in was removed - confirmed directly against the installed `Avalonia.Wayland` 12.1.0 package that it has **no Vulkan support at all** (zero Vulkan-related symbols in the DLL), while `Avalonia.X11` 12.1.0 has a full Vulkan backend (`X11RenderingMode`, `VulkanPlatformGraphics`, etc.). Leaving `UseWayland()` off means `UsePlatformDetect()` falls back to X11 (via XWayland under a real Wayland session, which is what this project's own dev machine - §1a - runs), the more mature and Vulkan-capable path of the two. The `Avalonia.Wayland` package reference is left in both `.csproj` files on purpose, so switching back is a one-line change if Avalonia's own Wayland backend catches up.
- **Vulkan itself is not currently enabled anywhere** - available as an explicit opt-in on the X11 backend (`X11PlatformOptions { RenderingMode = new[] { X11RenderingMode.Vulkan } }`, plus an `Avalonia.Vulkan` package reference), not adopted yet since it's still an immature path in Avalonia 12.x (known "renders nothing" bugs reported against recent preview builds) and wouldn't change anything for macOS regardless (Metal stays Metal either way).
