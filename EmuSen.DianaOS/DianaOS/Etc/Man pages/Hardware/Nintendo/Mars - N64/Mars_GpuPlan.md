...
| 7 — **the scan-out done 2026-09-21, `Mars_Gpu.md` §13**: all three games above full speed at four, the slowest at 120%; the average and sharing the image with the frontend not started | Optional: supersampling's average on the device; sharing the image with the frontend instead of readback | Only if phase 6's profile shows the readback or the average as the remaining cost |
