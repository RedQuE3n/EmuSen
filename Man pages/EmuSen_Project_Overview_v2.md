...
**Known remaining gap:** the shader-active path (F8 - Scanlines/Crt) still uses the old per-frame-offscreen-surface pattern, since a shader needs to sample the already-upscaled image in real output-pixel space (see the code's own comment on why). Not fixed the same way as the no-shader path above - worth doing (e.g. building the shader directly against `sourceImage` with a local-matrix scale instead of a separate offscreen upscale pass) at some point, but see the second, more urgent bug just found in this same code path below.
