...
- **First real piece landed: a shell console window.** `EmuSen.Mistress9` had *zero* debug-tooling integration before this - no watch/cheat/disasm/command-prompt UI at all, only ROM loading, save states, controller rebinding, and session preferences. `Settings > Shell Console...` now opens a real terminal-style window (`Views/ShellConsoleWindow.axaml`) onto the exact same `EmuSen.Shell.ShellInterpreter` the F4 console prompt and `EmuSen.Pharaoh90`'s `--commands` scripting use - same quoting/variables/pipes/redirection/if-for-while feature set, not a cut-down GUI-only subset. Answers the previously-open question this bullet used to end on ("does `EmulatorSession` expose enough state to construct a debug target the way it already exposes `Bus` for input?") - **yes**: `EmulatorSession.Cpu`/`.Renderer` were added as the same kind of Venus-specific escape hatch `.Bus` already was, letting `MainWindow` build a real `SnesDebugTarget` the same way `EmuSen.Hotaru`/`EmuSen.Pharaoh90` do. Rebuilt fresh (losing `!N`/`!!` history/variables, an accepted tradeoff) on every ROM (re)load so a command never runs against a Cpu/Bus/Renderer left over from an already-swapped-out core.
- **Not yet done:** the rest of a real Mesen-style multi-pane debugger - register panels, a hex viewer, a disassembly view, sprite/palette viewers, an event log, a watch panel - still doesn't exist. The shell console covers all of that functionally today (`regs`, `mem`, `disasm`, `watch`, `sprites`, `pal`, ...), just as text in one window rather than dedicated graphical panels. Whether dedicated panels are still worth building on top of the console, or whether the console itself is "enough," is an open question for whoever picks this phase up next.
- Breakpoint pause/step (Phase 5) still isn't wired into this window or `EmuSen.Mistress9` generally - the shell's `bp add`/`list`/`remove` work through it (hit-counting, like `EmuSen.Pharaoh90`), but nothing drives an actual halt/resume loop from this frontend yet.
