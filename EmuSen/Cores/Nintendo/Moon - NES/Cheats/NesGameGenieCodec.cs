...
{
    // Decodes real NES Game Genie codes (Galoob) - see Moon_Cheats.md §2.
    public static class NesGameGenieCodec
    {
        // A letter's index is the nibble it decodes to; every variant orders its sixteen differently.
        private const string Alphabet = "APZLGITYEOXUKSVN";

        // Which bit of the assembled 32-bit code each output bit comes from, most-significant first.
        ...
        // Bit 5 of the value moves for an 8-letter code, because the compare byte takes the place it occupied.
        ...
        public static (int Address, byte Value, byte? Compare) DecodeFull(string code)
        {
            ...
            // Each letter contributes its nibble at its own 4-bit slot, so letter 0 is the low nibble rather than.
            int raw = 0;
            ...
            // The 15 address bits are an offset into the cartridge's half of the CPU map, which is why the code.
