...
### 2.9 Re-investigated: SMW coin-chime/title-music audio report — MesenCE comparison, the master-clock rewrite, and an open, unresolved ~7.5% pacing gap
...
**The real, quantifiable finding**: a systematic ~7.5-8.7% SPC700 audio-sample-generation-rate undershoot, measured via `EmuSen.HeadlessDebug`'s `audiodump` verb across short cumulative bursts from a fresh (unsaturated) audio buffer (1/5/10/20/40-frame checkpoints) to avoid the buffer-cap sampling artifact above: ~980-985 interleaved samples/emulated-frame actual, vs. ~1065.36/frame expected (`32000Hz * 2 stereo / 60.0988fps`) - present both before and after every change described below.
...
**Where this leaves things**: the ~7.5-8.7% gap is real, reproducible, and still unexplained. Since it survives a from-scratch rewrite of the 65816-side cycle accounting untouched, the CPU→SPC700 budget hand-off (`VenusCore.RunFrame`'s `scaledSpc700Cycles`/`Spc700.CycleBudget`) is very likely delivering the SPC700 its correct, full budget - meaning the loss most likely happens *after* that hand-off, somewhere in the SPC700's own instruction-cycle accounting or `SDsp.Tick`'s cycles-to-samples conversion (`Tick(int cycles)`, `cyclesPerSample = 1024000 / AudioSettings.SampleRate`), neither of which has had the same MesenCE-comparison treatment the DSP synthesis logic (§2's ruled-out items above) already got. **Not yet investigated**: whether `Spc700`'s own per-opcode `Cycles` table (parallel structure to the 65816's, `Spc700.OpcodeTable.cs`) has an equivalent systematic undercounting the SPC700 CPU itself, independent of anything on the 65816 side; whether `Dsp.Tick()` is called with the exact right cumulative cycle count over a real frame boundary (vs. losing or double-counting a remainder somewhere); and a direct side-by-side trace against MesenCE's own `Spc.cpp` master-clock-driven pacing loop (§ this doc's original architecture notes) rather than just comparing formulas on paper.
