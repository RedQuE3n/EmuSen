...
*(Living document — updated as the toolchain grows. This revision adds the watchpoint mechanism and the generalized `tile` command, retires `CoinTileDumpLogging` in favor of the latter, and adds a shared, core-agnostic `FrameCount` used to timestamp screenshots.)*
...
`Watches` here just returns `MemoryBus.DebugWatches` — the bus owns the registry directly (§3.5), same pattern as it owning `Ppu`.
...
- **The watch registry currently only hooks WRAM writes.** `Ppu`'s VRAM/CGRAM/OAM writes and the general CPU-bus/SRAM path don't call `DebugWatches.RecordWrite` yet — built for what the Yoshi investigation needed, with wiring in the other write paths as natural, additive follow-up whenever an investigation needs to watch one of them.
...
- **`RecordWrite(spaceName, address, value, contextFactory)`** — called from an emulation-side write path (currently only `MemoryBus.Write8`'s WRAM branch) for every write. Cheap to call even with zero watches registered (a quick scan over however many are active). When a write matches an active watch, it's both **printed live** (`[WATCH #id] ...` — so the existing "play, then grep the console log" workflow keeps working with zero changes) and **stored** in a bounded per-watch buffer (last 500 events), so it's also queryable later via `watch log` without needing a fresh log upload.
...
Core-agnostic on purpose, same as `IDebugTarget` — lives under `Debug/`, not `Cores/Snes/`, since nothing about it is SNES-specific. A future NES core's `MemoryBus` would own its own instance the same way and call `RecordWrite` from its own write path(s).
...
**F5**/**F9** in the console build; Save/Load State menu items in the Avalonia frontend. Backed by `Common/StateSerializer.cs` — a reflective binary serializer that walks fields (not properties), skipping anything marked `[SkipInState]` (dispatch tables, back-references, and debug-only bookkeeping like `Cpu.DebugCpu`/`MemoryBus.DebugWatches`).
...
- **Breakpoints / single-step / pause-resume** — needs real execution-loop support for pausing mid-frame, which doesn't exist today. (The watch registry in §3.5 covers *observing* writes; it doesn't pause anything.)
...
- **Watchpoints beyond WRAM** — VRAM/CGRAM/OAM and the general CPU-bus/SRAM write paths don't call into `WatchRegistry` yet (§3.4).
