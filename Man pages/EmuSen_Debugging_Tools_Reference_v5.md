...
Lives in `EmuSen.Shell` (not `EmuSen.Hotaru`, the only current caller) specifically so a future console-based entry point doesn't have to duplicate it - the same frontend-agnostic reasoning `ShellInterpreter`'s own header comment gives. **Not** what the eventual Avalonia GUI debug window (§7's roadmap) would use, though - a real text-box widget already owns its own key handling and would read `ShellInterpreter.History` directly from its own KeyDown handler instead of going through a from-scratch console reader built for a raw terminal.
