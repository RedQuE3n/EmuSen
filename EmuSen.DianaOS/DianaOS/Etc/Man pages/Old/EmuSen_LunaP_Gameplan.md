...
- **Not an HTML layout host.** *Settled 2026-08-09, when CSS themes landed and the obvious next question was asked.* Three distinct things get called "HTML support" here and all three were declined, for different reasons. **HTML as a window-layout format** would be a third way to spell a window beside XAML and the fluent surface, and §9's whole argument is that there is exactly one vocabulary — every fluent extension is named after the XAML attribute it sets. HTML also brings a flow/box layout model Avalonia does not have, so `div`→`StackPanel` is lossy in ways no test would catch. **A real embedded WebView** (CEF/WebView2) is 100–200 MB of native payload against the eight published RIDs, with no X11-under-Wayland story, and it would hand the launcher a browser engine to stay light around. **An HTML/Markdown document viewer** for the man pages is the one that is merely unbuilt rather than rejected — it is narrow, needs no engine, and touches neither the theme nor the layout vocabulary. If user-editable *layouts* are ever wanted (skinnable launcher screens), the shape to build is a markup over the existing control kit, spelled in LunaP elements, not HTML. The CSS theme format is the precedent: an allow-list is what made that safe and testable, and a whole language is not.
