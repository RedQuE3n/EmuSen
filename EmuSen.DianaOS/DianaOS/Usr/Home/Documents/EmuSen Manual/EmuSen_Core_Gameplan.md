...
*(Written to be self-contained: if you're a fresh chat session reading only this file, §0-1 should orient you before the phased plan starts. Companion docs in this same `Man pages` folder — `EmuSen_Project_Overview_v2.md` and `EmuSen_Debugging_Tools_Reference_v5.md` — have the full technical depth this doc deliberately doesn't repeat; check those for "how does X actually work" questions, this doc is for "what's next and in what order." Older versions of both live in `Man pages/Old/` for history only - don't treat them as current.)*

---

## 0. Orientation, if you're new to this

**EmuSen** is a SNES emulator in C# / .NET 10 — full 65816 CPU, SPC700+S-DSP audio, PPU (all 7 background modes plus Mode 7, sprites, hi-res, mosaic, windowing, color math), memory/DMA. Several sibling projects on disk (see `EmuSen_Project_Overview_v2.md` §1 for the full current list): `EmuSen/` (this doc's subject — the core), `EmuSen.Hotaru/` (the console build used for development), `EmuSen.Serenity/` (shared presentation/shader code), and `EmuSen.Mistress/` (an Avalonia GUI purpose-built for bug testers, renamed from `EmuSen.Frontend/`). The separate, later-stage multi-core/launcher ambition (an EmulationStation-style browsing UI) is deliberately **not** being built into `EmuSen.Mistress` — that vision is scoped for a distinct, not-yet-created project, so bug-testing tooling and a future polished launcher never have to share one codebase. See `EmuSen_Launcher_Multicore_Gameplan.md` (renamed from `EmuSen.Frontend_Multicore_Gameplan.md`) for that plan.
...
**Solid, verified, not under active question:** the CPU/SPC700 opcode tables (both verified 256/256 against oxyron.de), all 7 PPU background modes with correct per-mode bit depth and compositing order (verified this session against the SNESdev wiki's priority tables, with two real bugs found and fixed — Mode 0's missing BG4 priority split, Modes 2-5 using the wrong compositing order), sprite rendering (including a real Y-wraparound bug fixed this session), open-bus emulation, SRAM sizing/mirroring (the actual fix for Super Metroid's anti-piracy boot check), hi-res output (both the pseudo-hi-res interleave and true Mode 5/6 tile-pairing), and a debug toolchain (`IDebugTarget`/`SnesDebugTarget`/`DianaOSInterpreter`/`WatchRegistry`) built specifically to be core-agnostic and reusable.

**In progress as of when this section was last touched (status not reconfirmed since - `EmuSen_Project_Overview_v2.md` §7 item 1 flags this same investigation as "almost certainly" no longer the active thread; check there first):** SMW's coins and Yoshi don't render. Long investigation, substantially narrowed. Ruled out with real evidence: the sprite/tile rendering logic itself, the general DMA transfer mechanism, DMA source-address computation, and every write-path bug that could have hidden real writes from the debug watch (direct bank-$7E addressing, the WMDATA `$2180` port, DBR-relative absolute addressing, block-move, and the low-RAM `offset < 0x2000` path — all now correctly observed).
...
- ~~The `Renderer`/Raylib split.~~ Done, as part of `EmuSen.Hotaru`'s Raylib→Avalonia migration - full writeup lives in `EmuSen_Project_Overview_v2.md` §7 item 7, not duplicated here. The `Raylib_cs.Color` usage that deliberately stayed behind was itself removed on 2026-08-05 — `Venus_PPU.md` §15.1.
...
- **Breakpoints / single-step / pause-resume.** Done. `VenusCore.RunFrame()` can now halt mid-frame (`IsHaltedAtBreakpoint`/`HaltedAddress`), preserving all scanline-loop state so the next call resumes the same in-progress frame; `IDebugTarget.Breakpoints` (`BreakpointRegistry`) backs the console's `bp add`/`list`/`remove` commands (renamed from `break` - the shell's own `break`/`continue` loop-control keywords shadowed it) and F4 prompt `step`/`continue`. See `EmuSen_Debugging_Tools_Reference_v5.md` §3.1/§3.3 for the full picture. Not yet wired into `EmuSen.Pharaoh` (its `bp add` only counts hits, doesn't halt) or the Avalonia GUI (Phase 6, still not started).
- **Headless harness for AI-agent-driven debugging.** Done — `EmuSen.Pharaoh`, a console entry point loading a ROM and driving `SnesDebugTarget`/`DianaOSInterpreter` (originally `DebugCommandProcessor`, before the shell rearchitecture/DianaOS rename - see `EmuSen_Debugging_Tools_Reference_v5.md` §3.3) exactly as originally envisioned here, plus scripted input (`--tap`), save-state load/save, screenshot capture, windowed CPU/SPC700 tracing, and generic `DebugSettings` flag control from the command line. This became the primary tool for real investigation work (the LttP color-math/subscreen chase and the Super Metroid boot-hang investigation both ran almost entirely through it) rather than staying a one-off. Full reference: `EmuSen_Debugging_Tools_Reference_v5.md` §3.15.

### Phase 6 — The Avalonia GUI debug window

- **First real piece landed: a shell console window.** `EmuSen.Mistress` had *zero* debug-tooling integration before this - no watch/cheat/disasm/command-prompt UI at all, only ROM loading, save states, controller rebinding, and session preferences. `Settings > DianaOS Console...` now opens a real terminal-style window (`Views/DianaOSConsoleWindow.axaml`) onto the exact same `EmuSen.DianaOS.DianaOSInterpreter` the F4 console prompt and `EmuSen.Pharaoh`'s `--commands` scripting use - same quoting/variables/pipes/redirection/if-for-while/coreutils feature set (this shell is called **DianaOS** now, everywhere - see `EmuSen_Debugging_Tools_Reference_v5.md` §3.3), not a cut-down GUI-only subset. Answers the previously-open question this bullet used to end on ("does `EmulatorSession` expose enough state to construct a debug target the way it already exposes `Bus` for input?") - **yes**: `EmulatorSession.Cpu`/`.Renderer` were added as the same kind of Venus-specific escape hatch `.Bus` already was, letting `MainWindow` build a real `SnesDebugTarget` the same way `EmuSen.Hotaru`/`EmuSen.Pharaoh` do. Rebuilt fresh (losing `!N`/`!!` history/variables, an accepted tradeoff) on every ROM (re)load so a command never runs against a Cpu/Bus/Renderer left over from an already-swapped-out core.
- **Since then: a windowed `coretop` (htop-style live hardware dashboard) and a `pause`/`resume` pair** closed the one real gap a background-thread emulation loop plus a UI-thread console opened (a command could otherwise race `RunFrame()`'s own concurrent state mutation) - see `EmuSen_Debugging_Tools_Reference_v5.md` §3.17.
- **Not yet done:** the rest of a real Mesen-style multi-pane debugger - register panels, a hex viewer, a disassembly view, sprite/palette viewers, an event log, a watch panel - still doesn't exist. The DianaOS console covers all of that functionally today (`regs`, `mem`, `disasm`, `watch`, `sprites`, `pal`, `coretop`, ...), just as text (plus a couple of dedicated windows for `coretop`/`feed`) rather than a full set of dedicated graphical panels. Whether dedicated panels are still worth building on top of the console, or whether the console itself is "enough," is an open question for whoever picks this phase up next.
- Breakpoint pause/step (Phase 5) still isn't wired into this window or `EmuSen.Mistress` generally - DianaOS's `bp add`/`list`/`remove` work through it (hit-counting, like `EmuSen.Pharaoh`), but nothing drives an actual halt/resume loop from this frontend yet.
