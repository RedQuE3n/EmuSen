...
- [`Venus - SNES/`](Nintendo/Venus%20-%20SNES/) — **complete** (CPU/PPU/APU/Memory)
  ...
  - [`Venus_Referee.md`](Nintendo/Venus%20-%20SNES/Venus_Referee.md) — not a slice: the whole core read against the SNES_MiSTer FPGA core, with Mesen as the tie-break. §0 is the part to read first and its answer inverts the N64 pass's — this referee is mostly *not* transliterated from a software emulator, so its agreement is evidence. §2 is the nine findings where both references agree against Venus, four of which a reading can carry most of the way because they are dead code or an internal inconsistency; §5 is the three disputes it could not settle; §8 is its commit history read as a witness list, naming the game that forced each fix; §9 is the corrections it owes the other pages. This index lists four Venus pages and there are nine — `Venus_SA1.md`, `Venus_SuperFX.md`, `Venus_NecDSP.md` and `Venus_OBC1.md` have never been listed here.
...
- [`Mercury - GB-GBC/`](Nintendo/Mercury%20-%20GB-GBC/) — **complete** (CPU/PPU/APU/memory/colour/cartridge, plus the hardware-test and real-cartridge records). This line read "CPU, memory and cartridge documented; no PPU or APU yet" until 2026-09-15, which stopped being true on 2026-08-09; `Mercury_Gameplan.md` §1 is the current state and this index was simply not updated with it.
  - [`Mercury_Referee.md`](Nintendo/Mercury%20-%20GB-GBC/Mercury_Referee.md) — not a slice: the whole core read against the Gameboy_MiSTer FPGA core, with Mesen 2 as the tie-break. §0 weighs the referee row by row (its PPU, DMA and interrupt logic are its authors' own; its CPU timing is padded T80, its sound quirks and boot ROM SameBoy's); §2 is the defect candidates where both references agree against Mercury, none yet tested; §4 is the DMG-compatibility mode on a CGB in detail, for the model choice; §9 maps every failing corpus ROM to a mechanism.
- [`Mars - N64/`](Nintendo/Mars%20-%20N64/) — ~~**planned, no code.**~~ **Built from 2026-09-15, and a frontend can open a `.z64` since 2026-09-18** ([`Mars_Core.md`](Nintendo/Mars%20-%20N64/Mars_Core.md)); two commercial games run in it headlessly ([`Mars_GameProbe.md`](Nintendo/Mars%20-%20N64/Mars_GameProbe.md)), and its pages are indexed in its own [`README.md`](Nintendo/Mars%20-%20N64/README.md). [`Mars_Gameplan.md`](Nintendo/Mars%20-%20N64/Mars_Gameplan.md) is the construction plan: the LLE-versus-HLE decision and its reopening condition, the oracle question answered before Phase A, seven phases with what proves each one done, and (§8) where the plan expects to be wrong. The only core whose plan predates its first line of code.
