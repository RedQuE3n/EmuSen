...
**A Unix-shaped layout inside that same sandbox root (`man hier`), added once the Users feature (§3.3) gave the shell a real concept of "whose home directory is this."** `TryResolve` now treats a leading `/` as THIS shell's own root, chroot-style, rather than the real OS filesystem root - `cd /Logs` works from anywhere, matching how a real `chroot` makes `/` mean the chroot directory. This creates an ambiguity `TryResolve` has to resolve explicitly: a leading-`/` path that's already a real, fully-qualified path landing inside `RootDirectory` (the shape every `DianaOSSandbox.RootDirectory`-built test path takes, e.g. `Path.Combine(DianaOSSandbox.RootDirectory, "Logs", ...)`) is honored as-is rather than being re-rooted a second time; only a short virtual reference like `/Logs`, typed by a person or built as a bare string, actually goes through the chroot reinterpretation. `EnsureSkeleton()` creates the real directories this layout needs (`Games`, `Saves`, `Saves/Save States`, `Firmware`, `Cheats`, `Logs`, `etc/EmuSen`, `tmp`, all under the home the shell is rooted at) the first time any shell starts in the process, and the initial cwd is that home. `useradd` creates **no** directory - per-account homes were dropped after they accumulated 6,179 empty folders from test runs, and an account is now an identity only (see §3.3 and `man hier`).
