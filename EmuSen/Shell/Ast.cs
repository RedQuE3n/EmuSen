...
namespace EmuSen.Shell.Ast
{
    ...
    // per word. See ShellInterpreter.ExpandWord for where this matters.
    ...
    // ShellInterpreter's own comment on why per-command-ephemeral scoping
    // isn't implemented).
    ...
    // pipeline (attached here, not per-stage) - see ShellInterpreter's own
