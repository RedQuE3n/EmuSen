...
- **`EmuSen/`** — the emulation core only (CPU/PPU/APU/memory, `Common/`, `DianaOS/`, `Settings/`). A pure library — no `Main`, no window ownership of its own. This is the primary development/debugging environment — cores get built and verified here first, standalone, before either frontend is the main way of running them. **It has no rendering-framework dependency at all as of 2026-08-05**: the `Raylib-cs` package is gone, and Venus's compositor uses its own `internal readonly struct Rgba32` (`Venus_PPU.md` §15.1). Its only package references are `SkiaSharp`, for PNG encoding in `Common/Imaging/PngFile.cs`.
...
- **`EmuSen.Endymion/`** — the shared SDL3 device layer, and the audio/input counterpart to `EmuSen.Serenity`: `AudioPlayer` (SDL3 audio streams), `DynamicRateControl`, `LinearResampler`, and under `Input/`, `GamepadManager` and `GamepadBindingMap`. Both frontends carried near-identical copies until the SDL3 migration merged them into `EmuSen.Nehellania`; audio then moved out to Endymion, and on 2026-08-05 the gamepad half followed and Nehellania was deleted — see `EmuSen_Multicore.md` §9.2. Depends on `EmuSen.Galaxia` and SDL3 only; deliberately no Avalonia, since nothing in it owns a window.
...
- **Device I/O** (`EmuSen.Endymion/`) — real audio output and gamepad polling through SDL3, shared between frontends for the same reason presentation is: one implementation, not a hand-copied duplicate per GUI. See `EmuSen_Settings_Reference.md` §4.10.
...
│   │                                     #   input lives in EmuSen.Endymion/Input/ now, shared.
│   └── Views/
...
│
├── EmuSen.Endymion/                  # Shared SDL3 device layer (both frontends reference) - the
...
└── EmuSen.Mistress/                 # Avalonia GUI - renamed from EmuSen.Frontend, deliberately
    ...
    │   │                                 #   moved to EmuSen.Endymion/ (shared with Hotaru).
    ├── Settings/
...
7. ~~The `Renderer`/Raylib split~~ — done, as part of the Hotaru Raylib→Avalonia migration: `Renderer.DrawDebugPanels`/`Shutdown()` (the on-window debug-overlay drawing and its lazily-created GPU texture) are gone entirely, so `Renderer` no longer owns any window/texture at all, in any mode. It kept `Raylib_cs.Color` as its internal pixel type for a while afterwards — a deliberate, separate decision, on the reasoning that the goal had been removing Raylib's *window/rendering* ownership rather than "zero Raylib anywhere in the solution," and that a full `Rgba32` migration was zero-payoff busywork.

**That judgement was reversed on 2026-08-05, and the reversal is worth recording because the original reasoning was not wrong about the goal — it was wrong about the cost.** Keeping the struct kept the package, and the package shipped: 21 copies of `Raylib-cs.dll` plus native binaries for four platforms, including inside the published Avalonia frontend, for a type with four byte fields whose functions were never once called. The replacement is `internal readonly struct Rgba32` beside the renderer, verified digest-identical across eighteen ROMs. Full accounting in `Venus_PPU.md` §15.1. See `EmuSen_Frontend_Driver.md`'s own top-of-file revision note for the migration this finishes.
