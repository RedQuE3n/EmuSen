...
| 3 — **done 2026-09-21, `Mars_Gpu.md` §7 and §8**; copy mode moved to phase 4 with the other two-cycle work | Textures: the ring, tile descriptors, all formats and TLUT, filtering, level of detail, perspective | Recorded display lists of the three probe games identical for a frame each |
| 4 — **done 2026-09-21, `Mars_Gpu.md` §10**: all three recorded frames now identical at two and four with nothing declined; of the four carries the mode appears to have, only the first blend's memory-alpha weighting is real, and no primitive of the three frames uses it | Two-cycle, and the neighbour carries by whichever of §3's options measures better | Every scene in the threaded suite identical; the serialised share recorded |
| 5 — **done 2026-09-21, `Mars_Gpu.md` §11**: four games identical at two and four from boot; copy mode ported; the probe at one unchanged | Integration: a GPU back end behind `DpInterface.Scale`, flush rules, the scan-out reading the readback, the setting, fallback when no device | Majora's Mask and the three probe games play at 2× and 4×; the probe at scale one still identical |
| 6 — **done 2026-09-21, `Mars_Gpu.md` §12**: the criterion is not met, and the reason is that it conflated drawing with presentation; drawing at four now costs what drawing at one does, and the scan-out and readback are what remains, so the device stays and phase 7 becomes the work | Measurement and the decision | 2× within five per cent of 1×'s frame rate and 4× at full speed on this machine, or the plan is recorded as not worth its dependency and retired |
| 7 — **the scan-out done 2026-09-21, `Mars_Gpu.md` §13; the last batch and the walk left pending, §14**: all three games at 150% or more at four, and two faster than one for Mario and Ocarina; **the average on the device, over a raster the device keeps, §15**: antialiasing at four now at 174–248%; **sharing the image with the frontend measured and not built, §16** | Optional: supersampling's average on the device; sharing the image with the frontend instead of readback | Only if phase 6's profile shows the readback or the average as the remaining cost |
