...
## 12. Known per-game quirks

Emulation-accurate PPU/CPU behavior is the default and should stay the default. The one exception below is a targeted, opt-in compatibility patch for a single game, gated on the cartridge header title (`Cartridge.Title`) — not a general "if game X do Y" framework. Documented here specifically so it can be found and re-evaluated later.

### 12.1 Zelda: A Link to the Past — stuck overworld subscreen ("yellow bar") quirk
...
**Confirmed mechanism** (verified with per-pixel diagnostics — see `Renderer.Scanline.cs`'s `ColorMathBlendLogging`/`ColorMathBlendScanline` and `Renderer.Backgrounds.cs`'s `[BG1TILE]` trace):
...
**First attempt at a fix was wrong — real regression found and reverted.** The first version of `ApplyLttpOverworldSubscreenQuirk` re-ran this same allow-list check on **every frame** while the overworld module was active, not just at the transition. That broke a real, unrelated thing: `$1D` is a shared scratch variable, reused by Bank00's `PaletteFilter_WishPonds`/`PaletteFilter_Crystal` (subscreen/color-math setup for their own effects) and apparently by the storm-intro sequence's rain overlay too. A user report that a fresh, lamp-skipping save had lost its rain entirely — confirmed by reverting the fix and watching rain's absence disappear along with it, and by finding the `PaletteFilter_*` routines writing `$1D` in the disassembly — showed the continuous version was clobbering `$1D` every single frame regardless of who legitimately owned it at that moment, not just correcting the one stuck-torch case.

**Current fix, take two (`VenusCore.ApplyLttpOverworldSubscreenQuirk`):** edge-triggered instead of continuous. It tracks WRAM `$10` (the main module index) frame-to-frame via `_prevModule`, and only runs the allow-list check on the exact frame `$10` transitions **to** `0x09` (overworld) **from** something else — i.e. only at the same moment the game's own `$00:DA63` state would run. `_prevModule` starts at `-1` on `LoadRom()`/right after a `LoadState()`, and the check is skipped whenever `_prevModule == -1`, so loading a save that already has `$10==9` baked in does not retroactively fire the correction — this is a going-forward fix for live play, not a save-file repair tool. By construction (the whole function returns immediately unless this exact one-frame edge is detected), it cannot touch `$1D` during ordinary steady-state gameplay in any module, so `PaletteFilter_WishPonds`/`PaletteFilter_Crystal` and whatever drives the rain overlay are structurally untouched — verified by running 300 frames of movement on a save with no house-transition in the run and confirming `$1D` never changes.

**Open question — not fully resolved:** whether real hardware also gets stuck here (i.e. this is a genuine, obscure vanilla ALTTP bug, and EmuSen would be correctly replicating it) or whether EmuSen additionally fails to run the `$00:DA63` reset state on the house-exit transition (a second, EmuSen-specific bug stacked on top of the first). Attempts to directly observe the `$00:DA63`/`$1D` interaction on a live house-exit transition, scripted via the headless harness's `--tap`, were abandoned — getting Link's X position to land exactly on the door's one-tile-wide collision column via blind input-duration guessing (no visual feedback loop available headlessly) proved impractical; several attempts landed Link adjacent to the door but never through it. **If revisiting:** either do this test interactively (RaylibFrontend, where door alignment is trivial to eyeball and correct in real time), or extend the headless harness with a way to read Link's world X/Y (WRAM `$22`/`$20`) back out mid-script to close the loop on tap durations programmatically instead of by eye.

**To find an accurate fix later:** if it's ever confirmed real hardware does NOT get stuck (i.e. this is purely an EmuSen module-transition bug), the right fix is to find and correct whatever's actually wrong with the `$00:DA63` dispatch on the house→overworld transition, and `ApplyLttpOverworldSubscreenQuirk` can then be deleted.

### 12.2 Zelda: A Link to the Past — missing rain overlay (separate, still open)

Found while verifying 12.1's regression fix, on a fresh save sent by the user ("skipped the lantern, went right outside"): the overworld storm-intro rain simply doesn't render on this save, and never starts even after 90+ warm-up frames.

Confirmed **not** caused by 12.1's fix (reproduces identically on the commit before it existed). Root-caused as far as: ALTTP draws rain as individual raindrop tiles scattered through BG3's tilemap (VRAM `$C000`, sourced from a `$7F2000` WRAM buffer per the disassembly's own RAM map). Diffing the full 8KB BG3 tilemap between a raining save and this non-raining save shows the raining save has real raindrop tile indices (e.g. `0xBC`, `0xBD`) at scattered positions that the non-raining save instead fills with a plain, high-priority filler tile (`0x7F`). Every other PPU register checked (BGMODE, TM/TS, CGADSUB, CGWSEL, window registers, BG3 scroll, CGRAM) is byte-identical between the two saves — so this isn't a compositing bug, the raindrop tile data itself was simply never DMA'd into VRAM for whatever moment this save was captured at. Not yet investigated further: what WRAM flag gates that DMA transfer, or why it didn't fire for this save's context. `SnesDebugTarget.GetVideoRegisters()` now exposes BG3/BG4 scroll, `BG3SC`/`Bg34Nba`, and the window mask/position registers (`W12Sel`/`W34Sel`/`WObjSel`/`Wh0-3`) from this investigation, for whoever picks it up next.
