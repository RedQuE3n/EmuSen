...
namespace EmuSen.Serenity.Shaders
{
    ...
    public static class HandheldFilters
    {
        ...
        public static string MonochromePanelSksl(int lightest, int light, int dark, int darkest, double response) => @"
...
half4 main(float2 coord) {
    ...
    v += (Shade(history1.eval(coord).rgb) - v) * rt;
    v += (Shade(history2.eval(coord).rgb) - v) * rt * rt;
    v += (Shade(history3.eval(coord).rgb) - v) * rt * rt * rt;
    return half4(Palette(v), 1.0);
}
".Replace("RT", F(response)).Replace("P0", Rgb(lightest)).Replace("P1", Rgb(light)).Replace("P2", Rgb(dark)).Replace("P3", Rgb(darkest));

        // Dots with gaps showing the reflector behind them, and the offset shadow each dot casts onto it.
        public static string DotMatrixSksl(int background, double coverage, double shadowOpacity) => @"
...
const float shadowOpacity = SHADOW;
...
".Replace("BG", Rgb(background)).Replace("COVER", F(coverage)).Replace("SHADOW", F(shadowOpacity));

        // A colour panel: blended toward the last frame by the panel's response, then Pokefan531's measured matrix in linear light.
        public static string ColourPanelSksl(double[] matrix, double luminance, double response) => @"
uniform shader original;
uniform shader history1;

const float blend = RESPONSE;
const float lum = LUM;

half4 main(float2 coord) {
    float3 c = mix(original.eval(coord).rgb, history1.eval(coord).rgb, blend);
...
".Replace("RESPONSE", F(response)).Replace("LUM", F(luminance))
...
".Replace("FLOOR", F(floor)).Replace("GAIN", F(gain)).Replace("GAP", F(gap)).Replace("ROWDIM", F(rowDim));
        ...
        private static ScreenFilter Monochrome(string name, int p0, int p1, int p2, int p3, int off, double response, double shadow) => new(name,
            new[]
            {
                new FilterPass(MonochromePanelSksl(p0, p1, p2, p3, response), PassScale.Source, History: 3),
                new FilterPass(DotMatrixSksl(off, 0.86, shadow), PassScale.Viewport),
            },
        ...
        public static ScreenFilter GbcLcd { get; } = new("Game Boy Color LCD",
            new[]
            {
                new FilterPass(ColourPanelSksl(GbcGbaMatrix, GbcGbaLuminance, 0.33), PassScale.Source, History: 1),
        ...
        public static ScreenFilter AgbLcd { get; } = new("Game Boy Advance LCD",
            new[]
            {
                new FilterPass(ColourPanelSksl(GbcGbaMatrix, GbcGbaLuminance, 0.25), PassScale.Source, History: 1),
        ...
        public static ScreenFilter Ags101Lcd { get; } = new("Game Boy Advance SP (AGS-101) LCD",
            new[]
            {
                new FilterPass(ColourPanelSksl(Sp101Matrix, Sp101Luminance, 0.0), PassScale.Source, History: 1),
