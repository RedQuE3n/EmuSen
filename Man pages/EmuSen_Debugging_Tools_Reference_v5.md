...
*(Living document — updated as the toolchain grows. This revision: `EmuSen.Debug.Commands`/`Debug/Commands/` (and `Debug/Cheats/`, `IDebugTarget`, and every registry/helper alongside it) merged into `EmuSen.Shell`/`Shell/` — see §3.3's own note on why the split existed and why it's gone now; the debug `break` command was renamed to `bp` in the same pass, since it turned out to be shadowed and completely unreachable by the shell's own `break`/`continue` loop-control keywords once those landed (§3.3's command table, §3.17). Previous revision corrected the breakpoints/single-stepping section, which existed by the time this doc was last touched but was still described as future work; documented `GetApuRegisters()`/`regs`' new APU section; filled in the command table's missing rows (`framelog`, `callers`/`writers`/`readers`, `dump`/`load`, `cheat`); and added the two harness-level tools that had no coverage at all despite becoming primary tools in practice - `EmuSen.HeadlessDebug` (§3.15) and `EmuSen.Validation` (§3.16). Earlier revision added the watchpoint mechanism, the generalized `tile` command, the shared `FrameCount`/screenshot timestamping, and a full 65816 disassembler — retiring `CoinTileDumpLogging` along the way. Also reflects a later decoupling pass: `WatchRegistry` moved from being owned by `MemoryBus` to being owned by `SnesDebugTarget`, communicating through a new, debug-agnostic `IWriteObserver` hook instead of `MemoryBus` holding a concrete `WatchRegistry`/`Cpu` reference directly.)*
...
### 3.14 Cheat engine (`cheat`, `Shell/CheatRegistry.cs`, `Shell/Cheats/{ActionReplayCodec,GameGenieCodec}.cs`, `Shell/Commands/CheatCommand.cs`)
