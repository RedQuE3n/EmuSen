...
**Enabling is synced on open, not tracked.** Every item in this menu needs a loaded ROM, and the states that decide that — a load, a failed load, a close, a reset, a `CPU HALT` — arrive from four different places including the emulation thread. Rather than have each of them remember to update five `IsEnabled` flags, `SubmenuOpened` recomputes all of them from `_session is { IsRomLoaded: true }` at the moment the menu is actually opened. A menu that is closed has no state worth keeping correct.

**The pause check mark is set in exactly one place.** `Pause` is a `ToggleType="CheckBox"` item, and `SubmenuOpened` is the *only* code that assigns its `IsChecked`. This matters because pause has three other entry points (the `P` hotkey, the console's `pause`, the console's `resume`) and because Avalonia's `MenuItem` flips `IsChecked` itself as part of handling a click. If `PauseEmulation`/`ResumeEmulation` also wrote the property, a click routed through them would be toggled twice — once by us, once by Avalonia — and the mark would end up inverted. Syncing only on open sidesteps the ordering question entirely: whatever the built-in toggle does to the mark while the menu is open is discarded and recomputed next time it opens.
...
The slot submenu is rebuilt on every `SubmenuOpened` rather than once, so each entry can show that slot's file timestamp (or `empty`) as it stands right now, including for a slot saved a moment ago from the same menu. It is also built once at construction, because a `MenuItem` with no children renders as a leaf with no submenu arrow — an empty-until-hovered parent looks like a dead item. Picking a slot only *selects* it; `Save State`/`Load State` then act on it, and both re-label themselves with the slot number when the menu opens. Two clicks to save to a specific slot is the RetroArch model, and it keeps the hotkeys meaningful — an `F5` that could not know which slot you meant would be worse than one that follows the selection.

**Fullscreen (`View > Fullscreen`).** The `F11` hotkey's toggle, and nothing more; the check mark syncs from the live `WindowState` on open, so a fullscreen entered by hotkey shows correctly in the menu.
