...
### 3.17 Unix-like text tools — `echo`, `sed`, pipes, and `history` (`Debug/Commands/{EchoCommand,SedCommand,HistoryCommand,CommandHistory,ITextFilterCommand}.cs`, `Debug/ConsoleLineReader.cs`)

Added to make the F4 prompt (and any future frontend built on the same `DebugCommandProcessor`) feel more like a real Unix shell, since that's the interaction model this toolchain was already modeled on (§3.3's own header comment) — three small, composable tools plus the plumbing needed to make them actually useful together, not three standalone novelties.

**Pipes.** `DebugCommandProcessor.Execute` now splits a command line on `|` before dispatch. The first stage gets the normal treatment — full `IDebugTarget` access, `help`/`summary`'s special-casing, everything §3.3 already describes. Every stage after a `|` is a **text filter**: it receives the previous stage's output as a plain string and returns a new string, via a second, opt-in interface (`ITextFilterCommand.Filter(target, input, parts)`) most commands don't implement — `mem`/`regs`/`sprites`/etc. read from a live `IDebugTarget` and wouldn't mean anything fed a string instead, so piping into one of them is a clear error ("`'mem' can't be used as a pipeline filter`") rather than silently doing something unexpected. There's no quoting support (matching the tokenizer §3.3 already has), so a filter pattern that itself needs a literal `|` can't go through this today.

- **`echo <text...>`** — prints its arguments back, joined by spaces. Mainly useful as a script marker/separator, or as the source end of a pipeline for testing a filter (`echo foo | sed s/foo/bar/`). Also implements `ITextFilterCommand` (ignoring whatever's piped in, same as real `echo` ignoring stdin), so `<anything> | echo literal text` doesn't error out just because it's mid-pipeline.
- **`sed s/pattern/replacement/[gi]`** — a basic, not-a-full-clone substitution filter: only `s///` (no line addressing, no hold space), pattern is a .NET regex, replacement supports `&` (whole match) and `\1`-`\9` (capture groups) translated from sed's own syntax. The delimiter can be any character right after `s` (`s#/bin#/usr/bin#` works, with `\<delim>` inside a field meaning a literal delimiter), same as real sed. Applied **per line** of whatever text it's given — without `g`, only the first match on *each line* is replaced, not just the first match in the whole piped blob, since most of what actually gets piped through this (`regs`, `snapshot list`, `channels`) is multi-line. Also callable standalone (`sed <expr> <text...>`) when there's nothing worth piping from.
- **`history`** / **`history clear`** — lists (or forgets) every non-empty command line submitted this session, numbered like bash's own builtin. Backed by `CommandHistory`, a small shared-state object `DebugCommandProcessor` writes to on every `Execute()` call and both `HistoryCommand` and `ConsoleLineReader` (below) read from — same "neither side owns it" pattern `SnapshotStore` already uses between `snapshot`/`diff` (§3.10).
- **`!N` / `!!`** — bash-style history expansion, handled by `DebugCommandProcessor.Execute` itself before any command ever sees the line (the same point real shells expand `!`-references): `!!` re-runs the last command, `!N` re-runs history entry `N` (1-based, matching `history`'s own numbering). The *expanded* text is what gets recorded into history, not the literal `!N`/`!!` typed - matching bash, and meaning `history` always shows what actually ran.

**Up/down-arrow recall (`ConsoleLineReader`).** `Console.ReadLine()` gives no line editing beyond whatever the terminal itself does, and no history recall at all - readline's job in bash, with no .NET equivalent. `ConsoleLineReader.ReadLine(history)` is a small, deliberately "basic" replacement (reads one raw key at a time via `Console.ReadKey(intercept: true)`, manually echoes/redraws) supporting cursor movement (arrows/Home/End), insert/Backspace/Delete, and Up/Down history recall against the exact list `history`/`!N` already use - press Up to walk backward through everything typed this session, Down to walk back forward, past the newest entry to whatever you'd been mid-typing before the first Up press (same behavior real readline gives you). Falls back to plain `Console.ReadLine()` when stdin is redirected (`Console.IsInputRedirected` - a script/CI feeding input via a pipe), since `Console.ReadKey` throws in that mode and there's no "up arrow" to recall from a non-interactive stream anyway.

Lives in `EmuSen.Debug` (not `EmuSen.RaylibFrontend`, the only current caller via the F4 prompt's `RunDebugPrompt`) specifically so a future console-based entry point doesn't have to duplicate it - the same frontend-agnostic reasoning `DebugCommandProcessor`'s own header comment gives. **Not** what the eventual Avalonia GUI debug window (§7's roadmap) would use, though - a real text-box widget already owns its own key handling and would read `DebugCommandProcessor.History` directly from its own KeyDown handler instead of going through a from-scratch console reader built for a raw terminal.
