//! The C# `StateSerializer`'s encodings: little-endian, a bool as one byte, arrays with no length. See Mars_Native.md §5.1.
...
impl StateError {
    pub fn status(&self) -> i32 {
        match self {
            ...
            StateError::NotAMarsState(_) => -3,
            StateError::Version(_) => -4,
            StateError::RdramSize(_) => -5,
            StateError::PendingOverflow(_) => -6,
            StateError::BufferTooSmall { .. } => -7,
            StateError::PendingInState(_) => -8,
        }
    }
}
