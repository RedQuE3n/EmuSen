...
**The general lesson, worth acting on before it bites again:** the infrastructure projects (Galaxia, Serenity, Cauldron, Endymion, Hotaru, Mistress, Pharaoh, WiseMan, LunaP, DianaOS) draw from the same Sailor Moon namespace as the cores, but this document only ever tracked the cores. That is how one name ended up claimed twice without anyone noticing. Either list the infrastructure names here too, or check both pools before reserving a new codename. (`Tomoe` was on that list too, until §3.16 of `EmuSen_Debugging_Tools_Reference_v5.md` folded that project into `EmuSen.Pharaoh` — a name leaving the infrastructure pool is exactly as worth recording here as one entering it. **`Crystal` and `Nehellania` both left the same way on 2026-08-05** — Crystal folded into the two cores (`EmuSen_Multicore.md` §9.1), Nehellania into `EmuSen.Endymion` (§9.2). All three names are free again.)
...
## 12. `Pegasus` — the shared notepad, claimed here and now hosted elsewhere

**`EmuSen.Pegasus` is the live collaborative notepad this project is developed through** — two people editing one document at once across two machines, backed by a CRDT so that neither can lose work. Named for Pegasus/Helios, who reaches people who are not in the same place by appearing in their dreams; the reach across distance is the whole point of the tool.

**It no longer lives in this repository.** It moved to <https://github.com/RedQuE3n/EmuSen.Pegasus>, where its design record is `docs/Pegasus_Design.md`, `docs/Pegasus_Format.md` and `docs/Pegasus_Sync.md`. The section is kept rather than deleted because **the name is still claimed** — the assembly is still `EmuSen.Pegasus`, still drawn from this pool, and a future core or tool must still not collide with it. A name outlives the directory it was claimed for.
...
It also passes §11's harder test, the one about whether a name is needed at all. Pegasus arrived from outside as four projects and was collapsed to **two** on the way in — `EmuSen.Pegasus` and `EmuSen.Pegasus.Tests` — because nothing outside it consumes its document model or its transport, and F#'s compilation order already enforces the layering the extra assemblies were bought for. Both names went with it; neither is free for reuse here.

**Two things about this entry are unlike every other name here.** It *was* the only project in the repository written in F#, and its departure takes `FSharp.Core` out of this solution entirely — which retires the `F#ascent` objection here by removing its subject rather than by answering it. And it is the first name claimed for something that is not part of the emulator at all: Pegasus is a tool the project is built *with*, not a component of what the project builds. That distinction is what eventually justified the separate repository, and if a second such tool ever arrives, this section is the precedent for both halves — keep it in the same pool, and expect it to leave.

One tie remains, and it is a real constraint rather than a courtesy: Pegasus consumes `EmuSen.LunaP` as a NuGet package. LunaP now has a consumer outside this solution, which `EmuSen_LunaP.md` §17 records.
